...
class Database {
  ...
  async viewEmployeesByManager() {
    ...
    const [managerRows] = await this.pool.execute(
  ...
  async viewEmployeesByDepartment() {
    ...
    const [departmentRows] = await this.pool.execute(
  ...
  async addEmployee() {
    ...
    const { firstName, lastName, roleOfEmployee, managerOfEmployee } =
      respondWith; // Destructure the response
  ...
  async updateEmployeeRole() {
    ...
    const selectedEmployee = employeeChoices.find(
      (employee) => employee.value === respondWith.employeeToUpdate
    ).name; // Get the name of the employee selected

    const { employeeToUpdate, roleToUpdate } = respondWith; // Destructure the response

    await this.pool.execute(`UPDATE Employees SET role_id = ? WHERE id = ?`, [
      roleToUpdate,
      employeeToUpdate,
  ...
  async updateEmployeeManager() {
    ...
    await this.pool.execute(
      ...
      [managerToUpdate, employeeToUpdate]
    ); // Update the employee's manager in the database

    console.log(
      "\x1b[32m%s\x1b[0m",
      `You have updated ${selectedEmployee}'s Manager to ${
        managerChoices.find((choice) => choice.value === managerToUpdate).name
      }`
    ); // Display a message to the user
  ...
  async deleteDepRolEmp() {
    ...
    // Catch the error for deleting a department if there are roles or employees in the department
    try {
      ...
      if (response.delete === "A Department") {
        ...
        await this.pool.execute(`DELETE FROM Departments WHERE id = ?`, [
          departmentToDelete,
        ...
        console.log(
          ...
          `You have deleted ${
            departmentChoices.find(
              (choice) => choice.value === departmentToDelete
            ).name
          } from the database.`
        ); // Display a message to the user
      }
      ...
      return console.log(
        "\x1b[31m%s\x1b[0m",
        "You cannot delete a department that has roles or employees associated with it. You must move or delete those roles and employees first."
      );
    }

    // Catch the error for deleting a role if there are employees in the role
    try {
      // If the user selects role
      if (response.delete === "A Role") {
        // Get all the roles from the database
        const [roleRows] = await this.pool.execute("SELECT * FROM Roles");
        ...
        await this.pool.execute(`DELETE FROM Roles WHERE id = ?`, [
          roleToDelete,
        ...
        console.log(
          "\x1b[32m%s\x1b[0m",
          `You have deleted ${
            roleChoices.find((choice) => choice.value === roleToDelete).name
          } from the database.`
        ); // Display a message to the user
      }
    } catch (error) {
      // Catch the error for deleting a role if there are employees
      return console.log(
        "\x1b[31m%s\x1b[0m",
        "You cannot delete a role that has employees associated with it. You must move or delete those employees first."
      );
    }

    // If the user selects employee
    if (response.delete === "An Employee") {
      // Get all the employees from the database
      const [employeeRows] = await this.pool.execute("SELECT * FROM Employees");
      ...
      await this.pool.execute(`DELETE FROM Employees WHERE id = ?`, [
        employeeToDelete,
      ...
      console.log(
        "\x1b[32m%s\x1b[0m",
        `You have deleted ${
          employeeChoices.find((choice) => choice.value === employeeToDelete)
            .name
        } from the database.`
      ); // Display a message to the user
    }
    ...
    // If the user selects return to main menu
    if (response.delete === "Return to Main Menu") {
      return; // Return to the main menu
    }
  }
